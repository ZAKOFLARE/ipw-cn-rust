...
export default defineEventHandler(async (event) => {
    ...
    if (!slug || Object.keys(slug).length === 0 || Object.keys(slug).length > 2) {
    ...
    const nodes = [...config.UDPing.DualStack, ...config.UDPing.IPv4, ...config.UDPing.IPv6]
    ...
    data = await $fetch(`${apiBaseUrl}v1/udping/${raw}${queryString ? '?' + queryString : ''}`, {
        method: 'GET',
        headers: {
