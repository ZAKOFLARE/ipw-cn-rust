...
export default defineEventHandler(async (event) => {
    ...
    if (!slug || Object.keys(slug).length < 2 || Object.keys(slug).length > 3) {
        throw createError({ statusCode: 400, statusMessage: 'Invalid slug' })
    }

    const backendID: any = slug[0]
    const type: any = slug[1]  // v4 or v6
    const raw: any = slug[2] || ''

    if (!['v4', 'v6'].includes(type)) {
        throw createError({ statusCode: 400, statusMessage: 'Invalid speed test type, must be v4 or v6' })
    }

    const nodes = [...config.SpeedTest.DualStack, ...config.SpeedTest.IPv4, ...config.SpeedTest.IPv6]
    const map = new Map<string, string>(nodes.map(node => [node.id, node.url]))
    ...
    data = await $fetch(`${apiBaseUrl}v1/speed/${type}/${raw}`, {
        method: 'GET',
        headers: {
